...
public class AnkiConnectClient
{
	...
	{
		var action = new AnkiAction("createDeck", new { deck = deckName }, null);
		await PostAsync(action);
	}

	public async Task<IReadOnlyCollection<long>> AddDynamicNotesAsync(IReadOnlyCollection<DynamicFlashcard> flashcards,
		string deckName, IReadOnlyCollection<string> tags)
	{
	...
	public async Task<AnkiNotesInfoResult> GetNotesInfoResilientAsync(IReadOnlyCollection<long> noteIds)
	{
		...
		catch (Exception ex) when (ex.Message.Contains("notes were not found"))
		{
			...
			if (succeededIds.Any())
			{
				...
				return new AnkiNotesInfoResult(recursiveResult.Succeeded,
					notFoundIds.Concat(recursiveResult.NotFound).ToArray());
			}
	...
	public async Task MergeTagsAsync(long noteId, IReadOnlyCollection<string> newSystemTags)
	{
		...
		foreach (var tag in newSystemTags) finalTags.Add(tag.Replace(' ', '-'));
		...
		try
		{
			...
			if (!response.IsSuccessStatusCode)
			{
				...
				throw new HttpRequestException(
					$"AnkiConnect request failed with status code {response.StatusCode}: {errorContent}");
			}

			var responseBody = await response.Content.ReadAsStringAsync();

			if (string.IsNullOrWhiteSpace(responseBody) || responseBody.Trim() == "null") return default;
